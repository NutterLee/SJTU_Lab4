...
Expression *readT(TokenScanner & scanner) {
   ...
   if (token == "-")
   {
	   string tmp_token2 = scanner.nextToken();
	   TokenType type2 = scanner.getTokenType(tmp_token2);
	   if (type2 == NUMBER)
	   {
		   string tmp_to_int = "-" + tmp_token2;
		   return new ConstantExp(stringToInteger(tmp_to_int));
	   }
